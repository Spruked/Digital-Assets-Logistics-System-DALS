...
(function() {
  ...
  function handleWebSocketMessage(data) {
    switch (data.type) {
      ...
      case 'comms_connected':
        addMessage('✅ Cali connected', 'system');
        break;
      case 'error':
        addMessage('❌ ' + (data.message || 'Unknown error'), 'error');
        break;
      default:
  ...
  function sendToWebSocket(data) {
    if (state.websocket && state.websocket.readyState === WebSocket.OPEN) {
      ...
      addMessage('⚠️ Connection lost, retrying...', 'warning');
      initWebSocket();
    }
  }
  ...
  function activateCali() {
    ...
    addMessage('👑 Cali_X_One activated', 'cali');
  ...
  function deactivateCali() {
    ...
    addMessage('🔄 Returned to worker team', 'system');
  }
  ...
  function speakResponse(text) {
    ...
    if ('speechSynthesis' in window) {
      ...
      utterance.onstart = () => addMessage('🔊 ' + text, 'speech');
      utterance.onend = () => {
  ...
  function setupEventListeners() {
    ...
    elements.orb.addEventListener('click', function(e) {
      ...
      if (state.isMuted) {
        stopListening();
        addMessage('🔇 Muted', 'system');
      } else {
        addMessage('🔊 Unmuted', 'system');
        startListening();
      }
    });
  ...
  function init() {
    ...
    if (!initSpeechRecognition()) {
      addMessage('⚠️ Speech recognition not available', 'warning');
    }
